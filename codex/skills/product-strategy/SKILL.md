...
Take a validated vertical from `market-pain` skill output through 12 sequential phases to a concrete, implementation-ready build plan. Owns the FULL journey from validated pain to launch blueprint.
...
### Start: `product-strategy [industry]` skill input

1. Check for an existing pain report (e.g., `[industry-slug]-pain-report.md` produced by `market-pain` skill)
2. If pain report missing or phases 1-4 incomplete: **STOP. Run `market-pain [industry]` skill input first.**
3. If valid: show executive summary + top problems, proceed to Phase 1

### Resume: `product-strategy resume [industry]` skill input
...
**Out of scope â€” run these companions separately if available:**

| Need | See also |
|-|-|
| Market research / pain validation | `market-pain` skill (required prerequisite) |
| Service packaging / pricing tiers | `ai-service-designer` skill |
...
| File | Use when |
|-|-|
| [`references/pricing-benchmarks.md`](references/pricing-benchmarks.md) | Running Phase 5. Competitor-pricing research protocol, hidden-pricing discovery, tier-spacing rules, setup-fee trade-offs, guarantee design, price-point validation, common pricing mistakes. |
| [`references/vertical-playbooks.md`](references/vertical-playbooks.md) | Any phase where the methodology feels abstract. Three worked end-to-end examples (accounting / trades / DTC subscription) distributed across US/UK/Canada â€” one clean GO, one KILL at Phase 3, one CONDITIONAL GO at Phase 11. Shows how gates protect the solo builder. |
...
# Evidence Ledger â€” [vertical] product strategy, $(date +%Y-%m-%d)
...
**All web research follows the Evidence Ledger Protocol** at this skill's `references/evidence-ledger-protocol.md` (self-contained â€” no cross-skill dependency; read once before any research). Three rules: **save raw â†’ rg or Select-String-verify quotes â†’ cite by `[E:S#]` Source #**.

---

## Phase 1: Pain Validation

**Gate-check only.** Verify `market-pain` skill phases 1-4 complete for this vertical.
...
4. **Ledger Integrity Check (mandatory)**: open the pain run's `evidence/evidence.md`. Verify (a) Index table exists, (b) for every `[E:S#]` cited in the pain report's Phase 1, 2, 2.5, 3 sections, the matching `## S#` entry exists in the ledger. Any missing entry = STOP and re-run `market-pain` skill with proper ledger discipline. `product-strategy` skill cannot proceed citing phantom ledger entries from the upstream skill.
5. **Output:** Summary table of qualifying problems with scores, gaps, economics
6. **Gate:** All market-pain phases done AND 3+ problems score 20+ AND ledger integrity check passed. Fail = **STOP, run `market-pain` skill first.** Counts feed from `evidence/evidence.md` ledger entries with `[E:S#]` tags, not from agent recall. `[A]` (assumption) entries don't count.
...
1. Identify dominant platforms from pain report or via verified web research (Perplexity + web fetch tool â€” save raw to evidence/raw/, then rg or Select-String-verify per references/evidence-ledger-protocol.md)
...
5. **Integration Viability Gate** â€” for EACH platform with >15% market share, document ALL of:
...
8. **GATE RULE:** If >50% of the addressable market uses platforms where write access is GATED or UNKNOWN, this is a **YELLOW flag** â€” note it prominently and proceed with caution. If >50% uses platforms where write access is confirmed READ-ONLY or no developer path exists, this is a **RED flag**. RED flags require explicit user acknowledgment before proceeding â€” the skill must **STOP and present the risk.** Counts feed from `evidence/evidence.md` ledger entries with `[E:S#]` tags, not from agent recall. `[A]` (assumption) entries don't count.

> **Measuring ">50%" correctly.** The rule measures **single-platform concentration**, not aggregate market share of read-only tools. If the dominant platform sits at 40% of the addressable market AND is confirmed READ-ONLY, that IS a RED flag â€” because 40% of the market uses ONE platform that blocks write-back, and those customers cannot be served. Do not aggregate scattered smaller tools to dodge the threshold. If any SINGLE platform holding â‰¥30% of the market is READ-ONLY with no write path, treat it as RED for that segment.
...
**REQUIRES verified web research for ALL pricing claims.** Use deepthink / extended thinking. Pull exact competitor tiers from their public pricing pages, not summaries â€” Perplexity for source-URL discovery, web fetch tool for the actual pricing page, nothing taken from AI-generated answers (save raw to evidence/raw/, then rg or Select-String-verify per references/evidence-ledger-protocol.md).

1. Bundle vs standalone analysis â€” which products are stronger together?
2. Run verified web research for competitor pricing (exact tiers, not estimates). Use Perplexity to find the pricing page URLs, then web fetch tool each page for verbatim tier/price data (save raw to evidence/raw/, then rg or Select-String-verify per references/evidence-ledger-protocol.md).
3. Design ascension path (entry â†’ mid â†’ premium â†’ managed)
4. Pricing: setup + monthly (never just one-time, per `ai-service-designer` skill rules)
...
8. **Weak-signal handling.** If a price point has only ONE competitor benchmark, that is weak signal â€” not a gate pass. Weak signal REQUIRES all of: (a) run all five hidden-pricing discovery techniques from `references/pricing-benchmarks.md` Section 3, (b) enter the weak-signal pricing into the Phase 7 risk register, (c) schedule a 90-day pricing re-evaluation based on close-rate / churn signals, (d) tighten the Phase 11 validation ramp (more customers, smaller price steps). Proceeding to Phase 6 on one benchmark WITHOUT completing a-d fails the gate.

**Deep-dive support:** [`references/pricing-benchmarks.md`](references/pricing-benchmarks.md) covers the full research protocol, hidden-pricing discovery techniques (G2 review mining, team-size proxies, investor leaks), tier-spacing rules (3-4Ã— multiplier), guarantee design, and the pass/weak/fail patterns for the 2-benchmark validation rule.
...
3. Team: roles needed at each scale (solo â†’ 10 â†’ 50 â†’ 100 customers)
...
8. **"TBD pending pilot" is still TBD.** Every line needs a current-best-estimate dollar figure grounded in published vendor pricing, research, or a real quote from a signed NDA. Post-pilot validation refines the estimate â€” it does not replace the estimate. The gate exists so Phase 12 can cross-check bundle pricing (Phase 5) against margins (Phase 6) with real numbers BEFORE demand validation in Phase 11. Skipping this breaks the whole phase sequence.
...
3. Competitive risks: **moat audit** â€” what stops a funded competitor copying this in 90 days?
...
4. **Pricing progression** â€” ramp methodology to discover price through early customers:
   ...
   - Stop raising when volume drops â€” that's your market price
   - Validation pricing â‰  launch pricing. Phase 5 benchmarks set the ceiling; this ramp finds the floor customers will accept
...
5. **Kill criteria:** Explicit STOP conditions (e.g., "if <2 of 5 calls show willingness to pay $X+, STOP"). Write criteria BEFORE the calls run â€” retro-fitting criteria to results destroys the discipline.
6. **Kill criteria format â€” MANDATORY.** Every kill criterion must contain: **(a)** a target count of calls or participants (e.g., "10 calls"), **(b)** a numeric count threshold (e.g., "â‰¥4 of 10"), **(c)** a single binary outcome per threshold (e.g., "willingness to pay $249+/mo"). Fuzzy quantifiers are rejected: no "some", "most", "moderate", "a few", "strong tone", "genuine interest", "open to the idea". If you cannot write a kill criterion with a count, a threshold, and a binary outcome, you have not defined a kill criterion â€” you have defined a hope. Phase 12 step 3 checks this explicitly.
7. **CONDITIONAL verdict:** If validation returns split signal â€” problem confirmed but pricing or target-segment unclear â€” do NOT force a binary GO/KILL. Return CONDITIONAL with a specific, bounded next action (e.g., "5 more calls in the X segment"). See `references/vertical-playbooks.md` Example 3 for the pattern.
...
8. Check: **Phase 2 Integration Viability** â€” verified write access path for platforms covering >50% of the addressable market AS DEFINED IN PHASE 1. Do not redefine the addressable market in Phase 12 to dodge the gate. If not verified, flag as gap and send back to Phase 2.
...
1. **No claim without source** â€” cite a phase number or a verified web source (URL fetched directly, not AI summary)
2. **`market-pain` skill is prerequisite** â€” Phase 1 gate-checks completion. No exceptions.
3. **Legal before pricing** â€” Phase 3 must complete before Phase 5
4. **Integration before architecture** â€” Phase 2 must complete before Phase 4
5. **Every builder phase (6-9) ends with concrete next steps** â€” not "consider" but "do X, here's how"
6. **Deepthink for configuration** â€” Phases 5, 6, 7 require extended thinking
7. **No pricing without verified research** â€” Phase 5 and any pricing claims require competitor URLs fetched directly, not AI-summarised

## Anti-Patterns

1. **Skipping to build plan** â€” Phases exist for a reason. No jumping to Phase 10.
2. **Estimated pricing** â€” "around $200/mo" fails Phase 5 gate. Get exact tiers by fetching competitor pricing pages directly.
3. **Generic SOPs** â€” "provide good support" fails Phase 8 gate. Specifics required.
4. **Orphan products** â€” Any service not tied to a 20+/30 pain point gets cut in Phase 4.
5. **Ignoring legal for speed** â€” Phase 3 is mandatory before pricing. RED blocker at Phase 10 wastes all downstream work.
6. **Copy-paste from pain report** â€” Synthesise and build on findings. Don't repeat them.
7. **Forgetting the human** â€” Phases 4, 11 generate scripts. The human executes calls. AI cannot validate demand.
8. **"TBD" in Resource Manifest** â€” Phase 6 requires dollar figures. Research until you have them.
