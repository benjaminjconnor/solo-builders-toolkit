...
Before any research starts, initialize the evidence ledger so every claim made downstream is rg or Select-String-verifiable.

```bash
mkdir -p evidence/raw
test -f evidence/evidence.md || cat > evidence/evidence.md <<'EOF'
# Evidence Ledger â€” [industry] in [geography], $(date +%Y-%m-%d)
...
**Read this skill's protocol reference before proceeding**: `references/evidence-ledger-protocol.md` (ships in this skill's `references/` folder â€” self-contained, no cross-skill dependency). Every research call in Phases 1â€“8 must follow it. The three rules in one sentence: **save raw â†’ rg or Select-String-verify quotes â†’ cite by `[E:S#]` Source #**. The protocol defines ledger entry format, the rg or Select-String-verify step, forbidden phrases, and the snippet-vs-page rule.
...
|-|-|-|-|
| 1 | Reddit | Industry subreddits + r/smallbusiness + country/region subs | **Use Reddit JSON API** (see protocol above). Search 3-5 industry subreddits + general business subs with pain-language queries. For high-scoring posts (50+ pts or 20+ comments), fetch thread comments for verbatim quotes. web search tool as fallback for discovery of relevant subreddits |
| 2 | Facebook Groups | Industry-specific groups | Chrome required â€” groups are often private. Search for group names first, then browse recent posts |
| 3 | Country-specific forums | Business + industry forums (e.g. Whirlpool in AU, UK Business Forums) | web search tool `site:[forum] [industry]` or Chrome navigate |
...
| 5 | G2 / Capterra | "What do you dislike?" sections for industry tools | web search tool or Chrome. Focus on dislike/cons sections specifically |
| 6 | Amazon | Reviews of business books in the category | Search "[industry] business books" â€” reviews reveal real problems. 1-3 star reviews are gold |
...
| 9 | LinkedIn | Posts from business owners venting about operations, asking for help, or discussing pain points | web search tool `site:linkedin.com [industry] [pain keyword]`. Fetch `/posts/` URLs with web fetch tool. `/pulse/` articles hit login walls â€” use search snippets only. Search terms: "[industry] frustrating", "[industry] business owner challenge", "[industry] missed calls", "[industry] admin". Also check for engagement signals (comments, reshares) â€” high-engagement complaint posts indicate widespread pain |

### Research Prompts

**Prompt A â€” `available research/search tools` (search_context_size: "high")** for THEME DISCOVERY ONLY:

> "What are [industry] business owners in [geography] complaining about? I want COMPLAINTS, FRUSTRATIONS, and WORKAROUNDS â€” not product features. Focus on: admin burden, missed revenue, staff friction, compliance headaches, customer communication failures, scheduling nightmares, technology frustrations, client dropout rates, retention problems, and burnout."
...
**Prompt B â€” Reddit JSON API** (see protocol above) for verbatim forum quotes. Run in parallel with Prompt A.

**Prompt C â€” `available research/search tools`** for software review complaints (follow Verified Research Protocol):
...
> â†’ web fetch tool top 2-3 review URLs â†’ extract verbatim "What do you dislike?" quotes with source URLs

**Prompt D â€” `available research/search tools`** for industry data to verify Prompt A themes:
...
> â†’ web fetch tool top 2-3 URLs â†’ extract real numbers and quotes, tag each `[E:S#]`
...
**Per-source ledger entry** (mandatory â€” see `references/evidence-ledger-protocol.md` for full format):

For each complaint, append a `## S{N}` block to `evidence/evidence.md` with URL, fetched date, raw file path, source type, author, score, created date, the verbatim quote, and the rg or Select-String-verify pattern + count. Then update the Index table at top.
...
Write the report with Phase 1 findings only and save it. The "no complaints found" result is valuable â€” it prevents wasted build time.
...
| **Buildability** | Requires hardware, regulation, or deep domain expertise | Moderate complexity, some integrations needed | Standard AI/automation solve â€” chatbot, workflow, integration |
...
- **Frequency cap on single-source evidence**: If a problem's evidence comes from fewer than 3 distinct sources (e.g., one Reddit thread + one review page only), Frequency maxes out at 2 regardless of raw mention count. One thread with 20 complaints â‰  a widespread problem. Three sources with 5 complaints each â‰¥ one source with 20.

**For anchored descriptions + worked cases per dimension, see `references/scoring-rubric.md`.** Use it when scoring a real vertical â€” the anchors remove drift between runs.
...
**Runs automatically after Phase 2 passes.** No skipping â€” pain without purchasing power is a trap.
...
### Rationalizations for skipping Phase 2.5 â€” all invalid

| Rationalization | Reality |
|-|-|
| "Phase 2 top score is 28+/30, economics obviously work" | High Phase 2 score measures PAIN INTENSITY, not purchasing power. Accounting example in `references/worked-examples.md` scored 27/30 and was killed at Phase 3 for economic reasons Phase 2.5 should have caught. Run Phase 2.5. |
...
**Phase 0 precondition check**: if `evidence/evidence.md` does not exist, run Phase 0 first. The optional delegated pass prompts below assume `evidence/raw/` is on disk; without it, raw saves silently fail.

### Research Architecture

Launch 4 parallel research streams. Each follows the Verified Research Protocol: `available research/search tools` â†’ `web fetch tool` top URLs â†’ save raw â†’ rg or Select-String-verify â†’ ledger entry `[E:S#]`.

**Each stream's optional delegated pass prompt MUST include**:
- "Save raw fetched content to `evidence/raw/{stream-name}-{source-slug}-{date}.md` BEFORE returning."
- "Return ledger entries in the format from `references/evidence-ledger-protocol.md`. Include the `rg or Select-String-verified` field with the pattern + count you actually ran."
- "If you cannot rg or Select-String-verify a quote, drop it â€” do not paraphrase."
- "Forbidden phrases (signal hallucination): 'Many users say', 'Industry studies show', 'Research suggests', 'Most {industry} businesses'. Do NOT cite snippets â€” web fetch tool every URL you plan to cite."

**After streams return**: main context re-runs `rg` or `Select-String` on each optional delegated pass-claimed quote against the saved raw file. Merge only verified entries into master `evidence/evidence.md`. optional delegated pass trust is verify-then-merge, not trust-and-merge.
...
```
### optional delegated pass re-verification attestation (Phase 2.5)

| Stream | optional delegated pass entries claimed | Re-verified by main context | Dropped (zero rg or Select-String matches) |
|-|-|-|-|
| Stream 1 | N | M | N-M |
| Stream 2 | N | M | N-M |
| Stream 3 | N | M | N-M |
...
If this attestation block is missing, Phase 2.5 is INVALID and the kill switch cannot be evaluated. No exceptions â€” the attestation is what makes the verify-then-merge rule mechanical instead of aspirational.

**Stream 1: Business Economics (Benchmarks + Industry Data)**

Run these `available research/search tools` queries:
...
Then web fetch tool the top 2-3 URLs from each search. Extract: (1) business count by size tier, (2) benchmark margins (cost of sales %, total expenses %), (3) revenue ranges by business size, (4) growth/decline trend. Tag every number `[E:S#]` or `[A]`.
...
Then web fetch tool each tool's ACTUAL PRICING PAGE (not review summaries). Extract: (1) exact pricing tiers per tool, (2) target business size, (3) any survey data on tech adoption rates. Tag every price `[E:S#]`.

**Stream 3: Problem-Specific Replacement Spend**

Run these `available research/search tools` queries (one per qualifying problem):
...
Then web fetch tool the top results. Extract per-problem: (1) current spend on workarounds, (2) answering service / VA pricing, (3) software that partially solves it + its price. The critical question: SWAP sell (existing budget) or NEW LINE ITEM ($0 current spend)?
...
Then web fetch tool top Reddit threads and review pages. Extract VERBATIM quotes about pricing with source URLs. Look for: psychological spending ceiling, what triggers resistance, ROI framing ("saves me X hours" vs "just another cost").
...
2. Navigate to the benchmark publication via available browser automation or web search tool
...
4. Calculate profit in actual dollars at representative turnover levels:
   - Solo operator ($150K): margin % Ã— $150K
   - Small business ($500K): margin % Ã— $500K
   - Medium business ($1M): margin % Ã— $1M

**Government pages are often 403-blocked** by scrapers and web fetch tool. Expect fallback to be the primary path. Try Chrome first, but don't spend more than 2 attempts.

**Fallback sources:** Industry body reports, publicly indexed summaries, bank benchmarking reports, insurance company surveys, accounting firm benchmark publications. Document which source was used and note "[primary] blocked â€” used [source]" in methodology.
...
- Current total software spend Ã— 20% (room for one more tool)
...
| Condition | Decision |
|-|-|
| No segment with â‰¥1,000 businesses can afford â‰¥$75/mo | **KILL** â€” unit economics don't work for a services business |
| Addressable segment (has pain AND can pay) < 1,000 businesses total | **KILL** â€” market too small for sustainable revenue |
| ALL qualifying problems have $0 replacement spend AND price ceiling < $100/mo | **KILL** â€” new budget line at low price is unsellable |
| Only viable segment is 20+ employees | **YELLOW FLAG** â€” enterprise sales cycle, different go-to-market required |
| Industry revenue declining > 5%/yr | **YELLOW FLAG** â€” businesses cutting costs, not adding tools |
| > 40% of market on pen-and-paper / no software | **YELLOW FLAG** â€” half the TAM hasn't crossed the "pay for software" threshold |
...
**Yellow flags don't kill** â€” they add context for the final decision. Two yellow flags together may effectively be a kill.
...
- Passed the Phase 2.5 economic viability check (viable segment â‰¥1K businesses at â‰¥$75/mo for their own top-problem variant)
...
**This step is non-negotiable.** Before researching individual competitors, identify the dominant industry platform's app marketplace (e.g. practice-management system app stores, service-trade platform marketplaces, property-management system app stores) and **visit the actual page** via Chrome or web fetch tool. Catalogue EVERY AI/automation tool listed â€” name, one-line description, category. This is the true competitive landscape, not what comes up in Google searches.

**Why**: Search results find the top 3-5 competitors. Marketplace pages reveal the full field. A search might show 10 apps â€” the actual marketplace might have 26+. Undercounting competitors leads to overconfident "gap" claims.

**Enforcement â€” MANDATORY field in Phase 3 output:**
...
- Audit method: [Chrome / web fetch tool / both]
- Ledger source: [E:S#] (raw file in evidence/raw/ contains the tool list â€” first 3 tool names: [name1, name2, name3])
```
...
For each competitor, visit these THREE pages (web fetch tool first, **available browser automation immediately if blocked**):
1. **Main website** â€” features, positioning, team
2. **Marketplace listing** â€” exact feature description as shown to buyers
3. **Pricing page** if separate

**optional delegated passes CANNOT use available browser automation.** If you delegate competitor research to optional delegated passes and they report a page as "inaccessible" or return only search-result summaries, YOU must follow up by opening the actual URLs in Chrome yourself. Do not accept "NOT FOUND" from a optional delegated pass without trying Chrome in the main context.
...
1. **Sprint test**: Could the nearest existing competitor close this gap in a single development sprint (2-4 weeks)? If yes, it's a **feature gap**, not a moat. Label it: "FEATURE GAP â€” low durability."
2. **Infrastructure test**: Does closing the gap require fundamentally different infrastructure (new data source, new platform, regulatory certification)? If yes, it may be a **structural gap** with higher durability. Label it: "STRUCTURAL GAP â€” medium/high durability."
3. **Data test**: Does the gap require proprietary data that competitors cannot easily acquire? If yes, it's a **data moat**. Label it: "DATA MOAT â€” high durability."
...
**Step 1 â€” Find competitors** via `available research/search tools`:
> `"[industry]" "[problem keyword]" software OR app OR tool OR platform [geography]`
> `"[industry]" AI OR automation "[problem keyword]"`

**Step 2 â€” Fetch each competitor's actual pages** via `web fetch tool` (Chrome fallback if blocked):
...
**Step 3 â€” Extract and tag**: competitor name, exact pricing `[E:S#]`, key weaknesses from real reviews `[E:S#]`, features users request. Do NOT trust search snippets for pricing â€” fetch the real page.

Apply Source Access Protocol â€” use Chrome fallback for any blocked sources.
...
**Top of Phase 3 report â€” MANDATORY audit confirmation** (without this the report is invalid):
...
- Pages visited per competitor: [typically 3 â€” website, marketplace listing, pricing]
```
...
### The Gap
[What users want that nobody provides â€” be specific. Cite `[E:S#]` or `[I:S#,S#]` for every user-want claim. Bare assertions like "users want X" without a ledger source = `[A]` and excluded from gap durability scoring.]

### Gap Durability
- **Rating**: FEATURE GAP / STRUCTURAL GAP / DATA MOAT
- **Sprint test**: Could [nearest competitor] add this in one sprint? [YES/NO â€” explain]
- **Infrastructure test**: Does this require fundamentally different infra? [YES/NO â€” explain]
- **Data test**: Does this require proprietary data? [YES/NO â€” explain]

### Your Opportunity
[How AI/automation could fill this gap â€” reference specific capabilities]
...
**Why this matters**: The marketplace audit catalogues what competitors DO. But marketplace apps cluster around obvious use cases (phone answering, reminders, notes). The real opportunities may be in categories nobody has entered â€” things that aren't on the marketplace because no competitor thought to build them.

**Process**:

1. **Map the industry workflow end-to-end**: Pre-visit â†’ Visit â†’ Post-visit â†’ Business ops. List every step where a human does repetitive work.
...
**The "Why nobody built it yet" column is critical.** If the answer is "because it's not valuable enough" â€” skip it. If the answer is "because it requires cross-system data that marketplace apps can't access" â€” that's a structural opportunity. If the answer is "because nobody thought of it" â€” it's a feature gap that will be copied quickly.
...
**Evidence Ledger applies differently to Phase 4.** Phases 1â€“3 ledger entries trace to fetched-and-rg or Select-String-verified raw files. Phase 4 calls have no equivalent mechanical proof (no recording-MCP exists in this skill). Therefore:
- Every Phase 4 quote cited in the final report is tagged `[A]` (assumption) â€” Phase 4 evidence cannot feed kill switches that require `[E:S#]`.
...
Phases 1â€“3 find what people *say* they want online. Phase 4 verifies what they *actually* pay for and use. **Do not build without Phase 4.**

### Rationalizations for skipping Phase 4 â€” all invalid
...
| "I'm confident in my segment" | Confidence is survivorship bias from your wins. The losses you don't remember look like "pain was clear online but nobody bought" â€” that's a Phase 4 kill you didn't run. |
...
| "I already know people in this industry" | If so, great â€” they're your first 3 calls. Do 7 more with strangers who have no social pressure to agree with you. |

**Kill Switch #4**: If you complete Phases 1â€“3 and skip Phase 4, you have NOT validated demand. You have a research report, not a build signal. Do not announce "demand validated" or commit to building until Phase 4 completes.

The only exception: you've already built the thing and have paying customers in this exact segment. If that were true, you wouldn't be running `market-pain` skill.

### Discovery Call Script

Generate a tailored script using the EXACT language from Phase 1 complaints. Not generic questions â€” specific to what was found.

**Opening (30 seconds):**
> "Hi, I'm [name]. I'm researching how [industry] businesses handle [broad problem category from Phase 2]. I'm not selling anything â€” just trying to understand what's actually frustrating for business owners. Mind if I ask a few quick questions?"
...
> `available research/search tools`: `"[industry]" software market share [geography] OR "[dominant platform]" users`
> `available research/search tools`: `"[industry]" "still using spreadsheets" OR "no software" OR "pen and paper" [geography]`
> â†’ web fetch tool top results. Extract platform names, user counts, pricing. Tag `[E:S#]`.

**Stream 2: Primary Platform API Deep Dive**
> `available research/search tools`: `"[dominant platform]" API documentation OR developer portal OR integration`
> â†’ web fetch tool the actual developer docs / API reference page. Extract: access level (open/gated), endpoints, auth method, webhooks, rate limits. Tag `[E:S#]`.

**Stream 3: Secondary Platform APIs**
> Same pattern for #2 and #3 platforms + any legacy systems.

**Stream 4: Build Stack Feasibility**
> `available research/search tools`: `"[key technology]" pricing [geography] OR "per minute" OR "per message"`
> â†’ web fetch tool each vendor's actual pricing page. Extract per-customer cost model. Tag `[E:S#]`.
...
> `available research/search tools`: `privacy law [geography] "small business exemption" AI OR automation`
> `available research/search tools`: `"[industry]" data privacy regulation [geography] OR "call recording" consent`
> â†’ web fetch tool the actual legislation pages or regulator guidance. Extract applicability, exemptions, requirements. Tag `[E:S#]`.

**Stream 2: Liability & Insurance**
> `available research/search tools`: `AI liability insurance [geography] "professional indemnity" OR "public liability" OR cyber`
> `available research/search tools`: `"[industry]" AI errors liability "consumer law" [geography]`
> â†’ web fetch tool insurer pricing pages and legal guidance. Tag `[E:S#]`.

**Stream 3: Industry-Specific Regulations**
> `available research/search tools`: `"[industry]" "[specific action e.g., automated communications]" regulations [geography]`
> `available research/search tools`: `spam act OR "do not call" [geography] automated messages business`
> â†’ web fetch tool regulator/legislation pages. Tag `[E:S#]`.
...
Launch parallel research streams â€” one per major competitor (max 4) + one strategic analysis. All follow Verified Research Protocol.

**Per-Competitor Stream:**
> `available research/search tools`: `"[competitor name]" features OR pricing OR review`
> â†’ web fetch tool their actual website, pricing page, and G2/Capterra review page
> â†’ Extract: exact features, exact pricing tiers `[E:S#]`, negative reviews verbatim `[E:S#]`, integrations, team/funding info
> â†’ For anything not on their website, try available browser automation

**Strategic Analysis (model synthesis, no external search needed):**
Using ALL verified data from Phases 1-7, produce:
1. KPI projections â€” customers needed for $10K/$50K/$100K MRR, realistic acquisition rate, churn estimate
2. Competitive advantages â€” what do you have that they don't? Be brutal.
3. Operational challenges â€” sole trader + 24/7 product + build-sell-support triangle
4. Kill risks ranked by likelihood Ã— impact
...
> `available research/search tools`: `"[service e.g., voice AI vendor]" pricing "per minute" OR "per message" OR "per call"`
> â†’ web fetch tool EACH vendor's actual pricing page. Extract exact tiers, per-unit costs. Tag `[E:S#]`.
> â†’ Calculate per-customer cost model from verified prices only.

**Stream 2: Architecture + 30-Day Build Plan**
> model synthesis (no search needed). Using verified vendor capabilities from Stream 1, produce:
...
> `available research/search tools`: `"[industry]" "[geography]" business directory OR association OR "member list"`
> `available research/search tools`: `"[industry]" outreach OR "cold email" OR "first customers" case study`
> â†’ web fetch tool top results. Extract real channels, directories, associations. Tag `[E:S#]`.
> â†’ Build GTM plan from verified channels only.
