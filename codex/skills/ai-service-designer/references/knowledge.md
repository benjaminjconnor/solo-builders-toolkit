# AI Service Designer â€” Knowledge Base

Conceptual foundations for designing AI services. Not a how-to â€” this is the "why" layer. Read this when a design decision feels arbitrary or you need to explain a trade-off to a client.
...
**Examples:**
- A bookkeeper's 20 hours of data entry â†’ 2 hours of review.
- A consultant's 12 hours of proposal drafting â†’ 5 hours.
- A property manager's 40 hours of after-hours standby â†’ passive monitoring.
...
**Trap:** the value isn't in the hours saved, it's in what those hours become. If freed hours go back into more of the same work, your service produces cost savings. If they go into higher-value work (client-facing, strategic, creative), your service produces capacity and revenue gain â€” and commands a much higher price.

### 1.2 Personalisation at scale

AI produces individually-tailored outputs at near-zero marginal cost. The surplus is the gap between "generic content" and "personalised-feeling content" Ã— volume.
...
**Service design implication:** pricing follows perceived personalisation Ã— conversion lift. If personalisation raises conversion by 10% on a $1M revenue line, the service can capture $10â€“30K/year of that lift.
...
- HITL is not optional in high-stakes outputs â€” it's the load-bearing wall.
...
**Practical heuristic:** "If an error cost more than 10Ã— the per-output fee, humans review."
...
- Chunking strategy matters â€” poor chunking breaks meaning.
- Per-call cost scales with tokens; services with long inputs become unprofitable fast.
- Caching (especially prompt caching) is a margin lever â€” build it in.
...
When something goes wrong â€” and eventually something will â€” you need a named human accountable for each material output. "The AI did it" does not stand up in regulatory proceedings or insurance claims.
...
Models drift. Data distributions shift. What worked at launch may not work in 3 months. HITL isn't just about catching errors â€” it's the mechanism that detects drift.

**Design implication:** even in low-stakes services, sample-audit 5â€“10% of outputs. The audit produces your quality signal over time.
...
Price against the client's current cost of the problem, not your cost of delivering the solution. If their current solution costs $40K/year and yours saves 80% of that with better quality, you can capture $15â€“25K/year.

### 4.2 Effort shape determines the model (not "hours")

See Frameworks Â§ F4. Hourly billing for AI services punishes efficiency. If you get fast at something, hourly billing reduces your fee. Fixed + retainer rewards efficiency.

### 4.3 Include the drift tax

Every LLM-based service has a drift tax: prompts degrade, models get deprecated, vendor pricing shifts. Bake ~10% of monthly fee into "ongoing maintenance" â€” not as a line item, but as margin that covers inevitable re-work.
...
**General rule:** if a regulator would look at the service and ask "who's accountable for this output?" â€” you need to have an answer before you sign a SOW.
...
Reality: clients at Level 4â€“5 (see Frameworks Â§ F2) often have strong opinions, existing tools, and internal teams. They buy specialist depth, not "AI help". Level 2â€“3 clients buy outcome-shaped services with less friction.

**Design implication:** most solo-operator AI services are best sold to Level 2â€“3 clients. Level 4â€“5 clients are available but demand deep specialisation (agentic systems, fine-tuning, evaluation pipelines) that usually exceeds a generalist's scope.
...
- "AI tools for your business" â€” describes nothing, closes nothing.
- "Document Automation" â€” describes an outcome, closes quickly.
- "Receptionist â€” After Hours" â€” describes a specific replacement for a specific role, closes fastest.
...
- Named service has a subject-matter domain attached. "Document Automation â€” Accounting" > "Document Automation".
...
Target ops overhead of 2â€“5 hours/month per client. Above that, margin breaks.
...
Without templates and packaged services, a solo operator tops out around 8â€“12 concurrent clients. With productisation, 25â€“40 is achievable. Beyond that, you're hiring.
...
4. Client's problem shape doesn't match any decision tree in `references/decision-tree.md` and you can't design one that fits.
...
7. Client expects a L1 â†’ L5 maturity jump in a single engagement.
