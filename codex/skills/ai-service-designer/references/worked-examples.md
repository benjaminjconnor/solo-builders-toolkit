# AI Service Designer â€” Worked Examples
...
## Example 1 â€” Ashworth & Keane Accountants (document-intake automation)
...
Maturity: Level 2â€“3 (they use a cloud accounting platform, Microsoft 365, Dropbox; Zapier for a couple of simple flows).

Urgency driver: one junior just resigned citing "soul-crushing admin". Partners don't want to replace her â€” they want to kill the work.

### Opportunity analysis

**Tree check** (`references/decision-tree.md` Tree 1): high volume, varied document structure, contaminates regulated records (tax returns). Recommends LLM-based extraction with schema, HITL review-before-send on high-stakes entries.

**F1 capability matrix:** extraction = green zone. Narrative reason for flagged items = green. Automated tax-filing submission = red zone â€” do not scope.

**Opportunity score (Template 1):**
- Time saved: 9 (~540 hours/year Ã— $75/hr internal cost = ~$40K cost base)
...
- Data readiness: 7 (docs exist but live in multiple places â€” lineage needs work)
- AI capability fit: 9 (green-zone with HITL)
- Delivery effort: 6 (moderate â€” needs integrations + UI)
...
- Competitive position: 6 (some tools exist â€” Dext, Hubdoc â€” but firm wants bespoke integration)

Weighted: 7.6/10 â†’ strong candidate.

### Service scope

**Service name:** Document Automation â€” Accounting (named service).
...
- Workflow: n8n self-hosted (own infra â€” keeps sensitive data in the client's data-residency region).
- Review UI: lightweight Next.js app on Cloudflare (build â€” this is our IP).
...
**Year 1 total** (at typical volume): $8,500 + ($1,200 Ã— 12) = $22,900.
...
- Margin on $1,200 monthly: ~9% â€” too thin.

**Re-priced:** monthly fee raised to $1,650. Margin ~34%. Still below 50% target â€” so we shift model to more setup-heavy ($12,000 setup, $1,650 monthly) after explaining the client saves ~$36K/year in junior cost regardless.
...
| R4 | Scope drift â€” partners want automated tax-filing submission next | Written change-order process; filing submission explicitly excluded |
...
- Junior hours on data entry: 60/month â†’ 7/month (steady state month 3).
- Junior retention: qualitative improvement â€” less boredom work.
...
- Error rate: target <2%. Actual first-month typically 4â€“5%, stabilising to <2% by week 8.
- Client ROI: payback within month 3 â€” $22,900 annual spend vs ~$40K saved junior cost + ~$60K latent revenue.
...
## Example 2 â€” Harbourview Property (call triage after-hours)
...
**Tree check** (`references/decision-tree.md` Tree 3): mix of booking-style and emergency calls. Named human on-call backup MUST be in scope. No fully-autonomous.
...
**Service name:** Receptionist â€” After-Hours Property (named).
...
**HITL pattern:** escalation-on-uncertainty. Any call the model isn't 80% confident about â†’ routes to emergency path by default (safer to over-escalate early).

**Tech stack:**
- Voice platform: Twilio Voice + Retell/Vapi-equivalent voice LLM (buy).
- LLM: a frontier model for classification (Claude, GPT-4-class, or equivalent â€” buy, abstracted so we can switch).
...
- Orchestration: n8n self-hosted (build â€” our IP).
...
**Year 1 total** (typical volume): $6,500 + ($950 Ã— 12) = $17,900.

**Margin check:** steady-state variable cost ~$280/month. Margin on $950 â‰ˆ 70% â€” healthy.

### Risk register (abbreviated)

| # | Risk | Mitigation |
|-|-|-|
| R1 | Missed emergency â€” classification fails or escalation breaks | Over-classify by default; 2-minute re-try loop; named backup PM; weekly simulation test |
| R2 | Wrong information given to tenant | Voice prompt restricts the model to triage and routing â€” no advice; transcripts audited |
...
- Client ROI: primary ROI is operational sanity + compliance, not revenue. Principal interviews describe "sleeping through the night for the first time in years" â€” that's the emotional hook for case-study content.

### Productisation note

Template the voice prompt for property management specifically. After three property management engagements, publish packaged pricing at $7K / $950. Adjacent verticals (real-estate sales, holiday rental) can reuse 70% of the setup â€” a useful adjacency.

---

## Example 3 â€” Copperline Consulting Group (proposal generation)
...
**Tree check** (`references/decision-tree.md` Tree 7): 65% reused, structured brief input, pricing logic partly rule-based, standard T&Cs. Strong fit for templated proposal-generation with LLM-assisted drafting + HITL review-before-send mandatory.
...
**Opportunity score (Template 1):**
- Time saved: 8 (4â€“6 hours per proposal Ã— 40 proposals/year = 200 hours/year at $300/hr internal = $60K)
...
- Delivery effort: 7 (moderate â€” content refactor + UI)
- Recurring-revenue shape: 8 (retainer-natural for content updates + prompt tuning)
- Risk profile: 8 (low â€” outputs always reviewed)
...
Weighted: 8.3/10 â†’ strong candidate.

### Service scope

**Service name:** Proposal Generation â€” Consulting (named).
...
- Review workflow: draft â†’ senior review â†’ partner sign-off â†’ PDF export + DocuSign send.
...
**Tech stack:**
- Frontend: Next.js on Vercel (build â€” our IP).
- LLM: Claude / GPT-4 via API (buy, abstracted).
- Content retrieval: Microsoft Graph API â†’ SharePoint (buy).
...
**Year 1 total** (typical volume): $18,500 + ($1,800 Ã— 12) = $40,100.
...
- Margin on $1,800: ~19% â€” too thin.

**Re-priced:** monthly raised to $2,500 after discovery demonstrates $60K annual time savings + $350K potential revenue recovery. Margin ~42% â€” close to target; accepted with understanding that some months (heavy content updates) will run closer to break-even.
...
| R7 | Scope drift â€” firm wants marketing content, case-study generation | Written change-order process; both explicitly out of scope |

### Expected outcome (60-day post-go-live)

- Proposal drafting time: 16 hrs â†’ 5.5 hrs (target); typical week-4 result 7 hrs dropping to 5 by week 8.
...
- Content library hygiene: as a side-effect, content gets refreshed and indexed â€” measurable reduction in "where's the latest version of X" slack searches.
