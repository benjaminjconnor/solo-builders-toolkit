# AI Service Designer â€” Frameworks
...
## Decision Tree â€” Which framework now?
...
| Real-time numeric calculation | Red | Green | Don't let an LLM do arithmetic â€” give it a tool |
...
## F2 â€” Automation-maturity ladder

**Use when:** you're proposing a service and need to make sure it meets the client where they are.
**Rating:** 5.

Five levels. Mismatch is the #1 failure mode â€” a Level 5 service sold to a Level 1 client will collapse within three months.
...
| 2. Templated | Discovery + scope reuse a template. Build reuses 60â€“80% of prior work. | Medium | 3â€“5 parallel engagements | After templates prove stable for 6 months |
...
- Most solo operators will never reach self-serve, and that's fine â€” packaged + retainer is a sustainable shape.
...
**Blended example â€” packaged service:**
- Setup fee: $3Kâ€“$10K (front-loaded engineering + discovery)
- Monthly fee: $500â€“$1,500 (ops, drift, minor enhancements)
- Per-unit overage: only if usage can spike dramatically (e.g. call minutes beyond a cap)

**Outcome-based warning:** requires all three â€” baseline, measurement, trust. Most of the time you're better off charging for effort and tracking outcome as a retention hook.

**Margin rule:** target 60â€“70% gross margin after tech-stack costs. If per-call LLM inference + vendor seats erode margin below 50%, re-price or re-scope.

---

## F5 â€” Risk-register structure
...
- **Call triage / voice:** misclassified emergency, missed escalation â†’ explicit escalation path with human backup.
- **Document extraction:** wrong-field extraction contaminates downstream records â†’ error-rate target + spot-audit schedule.
- **Sales follow-up:** off-brand voice, mis-targeted outreach â†’ brand-voice prompt + sample review cadence.
- **Content generation:** factual errors, plagiarism, trademark issues â†’ pre-publish checklist; never ship without human review.

Full risk-register template lives in `references/templates.md` Template 6.

---

## F6 â€” Buy vs build vs partner
...
## F7 â€” LLM vs classical ML decision
...
**Evaluation:**
- For LLM: prompt-based accuracy test on a held-out set of 50â€“100 examples. Track precision and recall.
- For classical: standard cross-validation, confusion matrix, feature importance.
- Ship with monitoring either way â€” drift is real and will hit you.

---

## F8 â€” Human-in-the-loop patterns

**Use when:** designing any service that produces outputs with non-zero cost-of-error.
**Rating:** 5.

Three patterns. Pick based on cost of error Ã— volume.
...
Rung 0: Free audit / assessment (lead capture)
    v
Rung 1: Entry project ($1Kâ€“$5K, 1â€“4 weeks)
    v
Rung 2: Flagship project ($5Kâ€“$25K, 4â€“12 weeks)
    v
Rung 3: Ongoing retainer ($500â€“$5K/mo)
    v
Rung 4: Premium advisory / exclusivity (5â€“10Ã— retainer)
```
...
- The ratio between rungs is typically 3â€“5Ã— (helps create contrast).

Full bundle structure and starter-kit logic in the pricing section of SKILL.md.

---

## F10 â€” Discovery cadence

**Use when:** scoping any engagement above a flat-fee threshold (say $5K+).
**Rating:** 4.

Two calls minimum. Free discovery is an investment, not a giveaway â€” but cap it.

### Call 1: Context call (30â€“45 min)
...
Discovery questionnaire template in `references/templates.md` Template 3.
